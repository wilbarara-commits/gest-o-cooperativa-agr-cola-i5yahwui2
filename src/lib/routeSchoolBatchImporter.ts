...
import { Contract, ContractSchoolLink, School } from '@/lib/types'
...
export function analyzeRouteSchoolsBatch(options: {
  ...
  const { fileName, parsedRows, targetRouteName, targetContract, allContracts, masterSchools } =
    options

  // Construir mapa de alocação de rotas para todas as escolas
    ...
    if (masterSchoolId) {
      ...
      if (alloc) {
        ...
        if (alloc.isTargetRoute) {
          ...
          // Pode atualizar dados do cadastro mestre se houver novos campos
        } else {
          // Bloqueio de exclusividade estrito: alocada a OUTRA rota
          isBlocked = true
          if (alloc.isCurrentContract) {
            blockReason = `Já alocada na rota "${alloc.rotaNome}" deste contrato.`
          } else {
            blockReason = `Já alocada na rota "${alloc.rotaNome}" do Contrato ${alloc.contratoNumero}.`
          }
        }
      }
    }
...
export async function executeRouteSchoolsBatchImport(options: {
  ...
  const { analysis, targetRouteName, targetRouteId, targetContractId, onProgress } = options
  ...
  for (let i = 0; i < eligibleItems.length; i++) {
    ...
    try {
      ...
      // 3. Vincular escola à rota no contrato
      if (schoolId) {
