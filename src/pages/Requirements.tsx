...
const MODULES: ModuleSpec[] = [
  ...
  },
  {
    ...
    route: '/pedidos',
    goal: 'Lançamento e acompanhamento do ciclo de vida dos pedidos com auditoria de regras de negócio e fluxo estrito de status.',
    details: [
      'Campos: id, numero, escola_id, ciclo_id (FK ciclos), origem (excel / whatsapp / manual), rota_id (FK rotas), validacao (json: status, motivo, detalhes), data_prevista, status (Pendente / Em Rota / Entregue / Cancelado), entregue_em, entregue_por (FK users), cancelamento_motivo.',
      'Fluxo de Status do Pedido: transições controladas na listagem de pedidos (menu por pedido: Pendente → Em Rota → Entregue, e Cancelado apenas enquanto Pendente).',
      'Cancelamento com Motivo Obrigatório: pedidos em fase Pendente podem ser cancelados mediante justificativa registrada para auditoria; pedidos cancelados são irreversíveis (não podem voltar para nenhum status).',
  ...
  },
  {
    ...
    route: '/rotas',
    goal: 'Organização logística de paradas, despacho e confirmação de entregas no campo.',
    details: [
      'Listagem de rotas por contrato contratual com paradas escolares ordenadas.',
      'Ao incluir pedidos na rota ou planejar entrega, transição direta de status de "Pendente" para "Em Rota".',
      'Botão "Confirmar Entrega" em cada parada da rota: muda o status do pedido para "Entregue", gravando timestamp exato (entregue_em) e usuário responsável (entregue_por).',
      'Baixa Automática de Estoque: confirmação da entrega abate imediatamente a quantidade de cada produto entregue do estoque da cooperativa, com garantia de idempotência.',
      'Atualização do Realizado Contratual: pedidos entregues alimentam em tempo real o realizado dos contratos e relatórios de execução.',
    ],
  },
  {
    id: '3.9',
...
const DATA_MODEL = [
  ...
  'pedidos (id, numero, escola_id → escolas, ciclo_id → ciclos, origem [excel/whatsapp/manual], rota_id → rotas, validacao [json], data_prevista, status [Pendente/Em Rota/Entregue/Cancelado], entregue_em, entregue_por → users, cancelamento_motivo)',
