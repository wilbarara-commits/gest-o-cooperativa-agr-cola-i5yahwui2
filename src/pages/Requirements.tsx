...
const MODULES: ModuleSpec[] = [
  ...
  },
  {
    ...
    details: [
      'Campos: id, nome, endereco, telefone, rota.',
      'Diretório cadastral base utilizado no relacionamento N:N com contratos e rotas logísticas.',
    ],
  },
  {
    id: '3.4',
    ...
    route: '/contratos',
    goal: 'Gestão institucional dos contratos PNAE/PAA, vínculo de N escolas participantes, rotas e cotas anuais por produto.',
    details: [
      'Relação N:N: As escolas são cadastradas dentro dos contratos através da collection contrato_escolas (contrato_id, escola_id, rota_id).',
      ...
      'Cota Anual por Produto: definição da cota em Kg/Unidade em contrato_itens para acompanhamento de execução.',
      'Relatório de Execução do Contrato: cards globais de Total Contratado, Total Realizado e % Execução, com detalhamento por escola participante (cota anual, realizado, valor realizado e % de execução).',
    ],
  },
  {
    id: '3.5',
  ...
  },
  {
    ...
    details: [
      ...
      'Matching Tolerante Inteligente: ignora acentos, caixa alta/baixa, espaços extras e trata apelidos entre parênteses (CEDAL, CEROM, CMEI Várzea).',
      'Regras Específicas: "EM PAULINO CUSTÓDUIO REZENDE" casa com "EM PAULINO CUSTÓDIO DE REZENDE"; "CM LAR DE ISABEL" e "EM LAR DE ISABEL" são mantidas estritamente separadas.',
      'Pré-requisito: Escolas já devem estar vinculadas ao contrato via contrato_escolas (o importador não cria escolas; gera pendência de vínculo).',
...
const DATA_MODEL = [
  ...
  'contrato_escolas (id, contrato_id → contratos, escola_id → escolas, rota_id → rotas) [Relação N:N]',
  'rotas (id, contrato_id → contratos, nome, ordem)',
  'contrato_itens (id, contrato_id → contratos, produto_id → produtos, preco, cota_anual)',
