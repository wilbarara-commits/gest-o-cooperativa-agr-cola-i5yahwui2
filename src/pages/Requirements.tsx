...
const MODULES: ModuleSpec[] = [
  ...
  },
  {
    ...
    details: [
      ...
      'Distinção Conceitual Rigorosa entre Rota da Planilha e Rota Logística: Rota (Planilha) representa a referência de origem da secretaria (abas da planilha centralizada); Rota Logística representa a divisão operacional de distribuição da cooperativa. Ambas são tratadas como atributos independentes.',
      'Listagem Compacta em Linhas (Tabela): Apresentação em linhas via componente Table do shadcn/ui para visualização de mais escolas por página (com paginação configurável: 10, 25, 50, 100 linhas), contendo colunas: Escola/Instituição, Tipo, Rota (Planilha), Rota Logística (com status de parada ou pendente de roteamento), Nº de Alunos, Telefone, Contratos Vinculados (badges) e Ações.',
      'Visão Detalhada da Escola Selecionada: Ao clicar em uma linha da tabela ou no botão "Ver detalhes", é aberto um painel/diálogo modal exibindo todos os dados mestres (endereço completo, telefone, e-mail institucional, tipo, alunos, rota padrão), bloco dedicado de Rota Logística de Distribuição (com posição de parada) e bloco de Contratos Vinculados & Rota da Planilha (collection contrato_escolas).',
      'Filtros Independentes por Rota (Planilha) e Rota Logística: Selects dedicados para filtrar por Contrato, por Rota (Planilha) e por Rota Logística (com opção explícita "Sem rota logística atribuída"), combináveis simultaneamente com a busca textual e com o filtro por Tipo de instituição.',
      'Importador Inteligente com Upsert (CSV/XLSX): Importação e reimportação de planilhas de escolas com atualização automática de dados existentes. Escolas já cadastradas não são ignoradas, mas marcadas no preview com status de "Atualização", exibindo comparativo de alterações campo a campo (ex.: tipo vazio → CRECHE, rota padrão, nº de alunos, contato). O usuário conta com seletor de regra de conflito: "Atualizar campos preenchidos no arquivo (manter valores existentes quando o arquivo estiver vazio)" [padrão seguro] vs "Sobrescrever todos os campos com os valores do arquivo". Resumo final detalha separadamente criadas, atualizadas, duplicadas no arquivo e erros.',
  ...
  },
  {
    ...
    details: [
      ...
      'Aba "Escolas" do Contrato rotulada "Rota (Planilha)": No vínculo da escola ao contrato, o campo de rota é estritamente rotulado "Rota (Planilha)", servindo de referência cadastral da secretaria. A Rota Logística NÃO é definida nesta tela.',
      'Detalhamento do Contrato com Seções Separadas: A visualização de detalhes do contrato divide explicitamente "Escolas por Rota (Planilha)" (contagem e relação por aba da secretaria) e "Escolas por Rota Logística" (paradas da cooperativa e relação destacada de escolas "Sem rota logística atribuída").',
  ...
  },
  {
    ...
    details: [
      'Conceito Central de ROTA LOGÍSTICA: Atributo operacional exclusivo de distribuição da cooperativa, definindo a sequência de paradas (ordem 1..N). Não se confunde com a ROTA DA PLANILHA (origem do pedido).',
      'Roteamento Baseado em Escolas: Atribuição e sequenciamento operam diretamente sobre as escolas vinculadas ao contrato (contrato_escolas.rota_logistica_id e paradas_rota), mantendo a rota da planilha intacta.',
      ...
      'Sequenciamento Drag-and-Drop de Paradas: Reordenação manual das escolas atendidas na rota gravada automaticamente na collection paradas_rota (rota_logistica_id, escola_id, ordem), com numeração visual (1, 2, 3...) e número de alunos como referência de volume.',
      'Despacho da Rota Inteira: Botão "Colocar em Rota" despacha todos os pedidos pendentes do ciclo ativo cujas escolas pertencem à rota (status Pendente → Em Rota), gravando registro em despachos.',
      'Entrega por Rota Inteira (Operação Offline da Central): Botão "Rota Entregue" marca todos os pedidos "Em Rota" dela como Entregue simultaneamente, com registro de conferente, baixa de estoque e liberação de atestos.',
...
const DATA_MODEL = [
  ...
  'rotas (id, contrato_id → contratos, nome, ordem) [Rotas da Planilha da Secretaria - Referência]',
