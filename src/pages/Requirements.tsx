...
const MODULES: ModuleSpec[] = [
  ...
  },
  {
    ...
    details: [
      ...
      'Sem Aba de Rotas no Cadastro de Contrato: A criação de contrato foca nas informações essenciais e na adição de itens. A aba de rotas do contrato atua como um resumo somente leitura das rotas e escolas configuradas, oferecendo atalho direto "Configurar no Roteamento & Despacho".',
  ...
  },
  {
    ...
    route: '/rotas',
    goal: 'Roteamento por contrato, sequenciamento drag-and-drop de paradas com volume de alunos, despacho e entrega por rota inteira da cooperativa.',
    details: [
      'Separação de Rotas: mantém a rota da planilha da secretaria (referência) e cria a Rota Logística da cooperativa (collection rotas_logisticas). O despacho e sequenciamento usam exclusivamente a rota logística.',
      'Primeira Configuração Direto na Tela de Roteamento: Ao selecionar um contrato sem rotas logísticas, a interface oferece a configuração imediata ali mesmo: criação de rotas com nome livre (quantas quiser, ou até o limite contratual se num_rotas_logisticas estiver definido), atribuição das escolas e pedidos do contrato e sequenciamento.',
      'Nomes de Rota Livres & Criação Dinâmica: botão "Nova Rota" permite adicionar rotas logísticas a qualquer momento com nome descritivo livre (ex: Rota Centro, Rota Rural, Setor Norte).',
      'Coerência de Dados com Contrato (contrato_escolas.rota_id): quando uma escola é atribuída a uma rota na tela de roteamento, o vínculo no contrato é preenchido e sincronizado automaticamente. A rota vive no vínculo escola↔contrato; a tela de roteamento a define de forma integrada.',
      'Sequenciamento Drag-and-Drop de Paradas: reordenação manual das escolas atendidas na rota gravada automaticamente na collection paradas_rota (rota_logistica_id, escola_id, ordem). Numeração visual para o motorista (1, 2, 3...) e exibição do número de alunos de cada escola como referência de volume.',
      'Despacho da Rota Inteira: botão "Colocar em Rota" exige apenas que a rota tenha paradas atribuídas e ordenadas (nada além disso do contrato), despachando todos os pedidos associados para "Em Rota" e gravando histórico na collection despachos.',
      'Entrega por Rota Inteira (Operação Offline da Central): botão "Rota Entregue" marca todos os pedidos "Em Rota" dela como Entregue simultaneamente, registrando entregue_em e entregue_por, com baixa de estoque automática e liberação de atestos.',
      'Tratamento de "Não Entregue": pedidos despachados que o motorista não entregou podem ser cancelados definitivamente pela central com motivo logístico fixo.',
    ],
  },
  {
    id: '3.9',
...
const DATA_MODEL = [
  ...
  'rotas (id, contrato_id → contratos, nome, ordem) [Rotas da Planilha da Secretaria]',
  'rotas_logisticas (id, contrato_id → contratos, nome, ordem, ativa [bool]) [Rotas Logísticas da Cooperativa]',
